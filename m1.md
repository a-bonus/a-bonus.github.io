...
2.  **Configure in "Variable View":**
    ...
    - **B. Set value labels (nominal and ordinal variables):** For each categorical variable, you must tell SPSS what the numbers in your data stand for. *Plain English:* you are building one small lookup table per column—each numeric code (1, 2, 3, …) gets a text label. Scale variables (e.g., age, minutes, hours of sleep) usually do **not** need this kind of value labeling.

      - Click the cell in the "Values" column for **one** categorical variable (e.g., `Employment Status`). A small button with three dots `...` will appear. Click it.
      1. In the dialog, enter a number in the `Value` box (e.g., `1`) and its text in the `Label` box (e.g., `Employed Full-Time`). Click **Add**.
      2. For **the same variable only**, enter the next value/label pair and click **Add** again. Continue until **every code used in your data** for that variable has a label (e.g., all employment options, all time zones).
      3. Click **OK** to close the dialog.
      4. Open **Values** for **each remaining** nominal or ordinal variable and repeat steps 1–3. The instructor may check labels for variables such as Employment, Computer, and Time Zone.
