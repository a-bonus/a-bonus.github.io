...
1.  **Null Hypothesis of the t-Test:**
    - `Think:` What does the null hypothesis claim about the relationship between two group means? What would it mean if employment status has "no effect" on age?
    - `Hint:` The null hypothesis states that the population means of the two groups are equal. Express this using the equals sign (=) with the appropriate population parameters (µ) for each group.
2.  **Effect Size (d):**
...
_Note: The screenshot above is only an example and does not use the actual variables from your assignment. Your output will show different values._
...
_Note: The screenshot above is only an example using sample variables. Your dialog should show Mem1 and Mem2 from your dataset._
